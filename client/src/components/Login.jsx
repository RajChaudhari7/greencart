...
const Login = () => {
    ...
    const handleSellerLoginClick = () => {
        setShowUserLogin(false); // Hide the user login form
        navigate('/seller'); // Navigate to the seller login page
    };
        ...
        <div onClick={() => setShowUserLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-30 flex items-center text-sm text-gray-600 bg-black/50'>
            <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] rounded-lg shadow-xl border border-gray-200 bg-white">
                ...
                <p className="mt-4 text-center w-full">
                    Are you a seller? <span onClick={handleSellerLoginClick} className="text-primary cursor-pointer">Login</span>
                </p>
            </form>
        </div>
    );
